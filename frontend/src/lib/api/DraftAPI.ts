...
export const publishDraftAsUser = async (uuid: string): Promise<{ id: number }> => {
    const res = await API.post<{ chart: { id: number } }>(ROUTE_CONFIG.draft.publishAsUser(uuid));
    return res.data.chart;
};

export const publishDraftAsGuest = async (uuid: string, chartBase: ChartBase): Promise<{ id: number }> => {
    const res = await API.post<{ chart: { id: number } }>(ROUTE_CONFIG.draft.publishAsGuest(uuid), { chartBase });
    return res.data.chart;
};