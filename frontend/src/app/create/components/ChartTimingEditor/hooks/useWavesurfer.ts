...
export function useWaveSurfer({
  ...
  useEffect(() => {
    ...
    ws.on('ready', () => {
      ...
      const shadow =
        ws.getWrapper().parentElement?.shadowRoot ?? (ws.getWrapper().getRootNode() as ShadowRoot);
      if (shadow) {
  ...
  useEffect(() => {
    ...
    const applyZoom = (newPxPerSec: number, pivotClientX: number) => {
      ...
      const ratio =
        (pivotClientX - rect.left + el.scrollLeft) / (wsRef.current!.getWrapper().scrollWidth || 1);
