...
export default function ChartLyricsEditor({
    ...
    useEffect(() => {
        onWordClickRef.current = onWordTimingClick;
    }, [onWordTimingClick]);
    useEffect(() => {
            ...
            line.items.forEach(item => {
                if (item.type === 'draft') {
                    ...
                    span.addEventListener('focus', () => {
                        span.style.borderBottomColor = '#a855f7';
                    });
                    ...
                    if (timing)
                        span.title = `${timing.start.toFixed(2)}s – ${timing.end.toFixed(2)}s`;
    ...
    function handleKeyDown(e: KeyboardEvent) {
        ...
        if (e.key === 'ArrowRight' && draftEmpty) {
            ...
            if (e.shiftKey) {
                ...
                cursorRef.current = hasSel()
                    ? selEnd()
                    : Math.min(tokensRef.current.length, cp + 1);
                anchorRef.current = null;
            }
            renderDOM();
            return;
        ...
        const onMouseUp = (me: globalThis.MouseEvent) => {
            ...
            if (!isDraggingRef.current && !me.shiftKey && clickedBrick) {
                ...
                if (wi !== undefined) {
                    const timing =
                        timingsRef.current[
                            tokensRef.current.slice(0, tokenIdx).filter(t => t.kind === 'word')
                                .length
                        ];
